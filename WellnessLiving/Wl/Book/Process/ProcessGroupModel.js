...
function Wl_Book_Process_ProcessGroupModel()
{
   ...
   * @property {string[]} a_signature Signatures for a purchase option that requires agreement to several distinct          contracts at once (for example, one per Tuition visitor).          Keys are contract keys in `{uid}-{id_purchase_item}-{k_id}` format.          Values are signatures of the client in base64 format.
