...
function Wl_Book_Process_Store_StoreGroupModel()
{
   ...
   * @property {number} i_session The number of sessions that this item can cover.   The same as in {@link Wl_Book_Process_Store_StoreGroupModel.a_purchase_item_check}.   `0` if not applicable.
   * @property {?string} k_login_promotion Login promotion key.    The same as in {@link Wl_Book_Process_Store_StoreGroupModel.a_login_promotion}.   `null` if not applicable.
   * @property {?string} k_login_promotion_guest_pass Host login promotion key that grants the guest pass.    `null` if not applicable.
   * @property {?string} k_session_pass Session pass key.    The same as in {@link Wl_Book_Process_Store_StoreGroupModel.a_session_pass}.   `null` if not applicable.
   * @property {?string} s_value The unique identifier of the purchase item.   The same as in {@link Wl_Book_Process_Store_StoreGroupModel.a_purchase_item_check}.   `null` if not applicable.
